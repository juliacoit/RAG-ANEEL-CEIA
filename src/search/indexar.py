...
indexar.py
==========
Search Engineer — Módulo de indexação

Lê os chunks gerados pela ingestion (chunks_completo_unificado.parquet),
gera embeddings via sentence-transformers (local, gratuito)
e indexa no Qdrant local (Docker). Cria também o índice BM25 esparso.
...
Uso — rodar da RAIZ do projeto:
  python src/search/indexar.py
  python src/search/indexar.py --limite 100   # teste rápido
  python src/search/indexar.py --resetar       # apaga e recria coleção
  python src/search/indexar.py --testar        # roda perguntas de teste
"""
...
def carregar_indices(qclient: QdrantClient) -> tuple:
    ...
    if not ARQUIVO_BM25.exists():
        raise FileNotFoundError(
            ...
            "Rode src/search/indexar.py primeiro."
        )
    with open(ARQUIVO_BM25, "rb") as f:
        bm25 = pickle.load(f)
    with open(ARQUIVO_IDS, "rb") as f:
...
def main():
    ...
    log.info("  from src.search.indexar import conectar_qdrant, carregar_indices, buscar")
